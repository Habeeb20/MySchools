...
const AllTutorial = () => {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                const response = await axios.get(`${import.meta.env.VITE_APIA}/tutorial`)
  ...
  return (
    <div className="flex h-screen">
        ...
        <nav className="flex flex-col space-y-2">
          ...
          <NavLink to="/admin/tutorial" className="flex items-center space-x-2 text-gray-300 hover:text-white px-4 py-2 rounded-lg bg-green-700 font-bold ">
      ...
      <h1 className="text-2xl font-bold mb-6">All tutorial</h1>
      {loading ? (
            ...
            <tr>
              ...
              <tbody>
                {tutorial.map((user, index) => (
                  <tr key={user._id} className="border-b">
                    <td className="py-2 px-4">{index + 1}</td>

                    <td className="py-2 px-4">{user.tutorialName}</td>
