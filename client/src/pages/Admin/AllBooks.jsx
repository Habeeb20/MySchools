...
useEffect(() => {
    const fetchUsers = async () => {
        try {
            const response = await axios.get(`${import.meta.env.VITE_APIA}/book`)
    ...
    <div className="flex h-screen">
      <aside className="hidden md:flex  fixed top-0 left-0 w-full md:w-44 bg-green-900 text-white flex flex-col h-screen mt-0 mb-0">
        ...
        <nav className="flex flex-col space-y-2">
          ...
          <NavLink to="/admin/books" className="flex items-center space-x-2 text-gray-300 hover:text-white px-4 py-2 rounded-lg  bg-green-700 font-bold">
            <AiOutlineExclamationCircle className="h-6 w-6" />
      ...
      <div className="flex-1 bg-gray-100 p-8 overflow-y-auto">
        <h1 className="text-2xl font-bold mb-6">All Book Stores</h1>
        ...
        {loading ? (
            ...
            <tr>
                  ...
                  <th className="py-2 px-4 text-left">bookshopName</th>
              ...
              <tbody>
                {books.map((user, index) => (
                  <tr key={user._id} className="border-b">
                    ...
                    <td className="py-2 px-4">{user.bookshopName}</td>
