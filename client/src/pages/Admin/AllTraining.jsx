...
function AllTraining() {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                const response = await axios.get(`${import.meta.env.VITE_APIA}/training`)
  ...
  return (
    <div className="flex h-screen">
        ...
        <nav className="flex flex-col space-y-2">
          ...
          <NavLink to="/admin/training" className="flex items-center space-x-2 text-gray-300 hover:text-white px-4 py-2 rounded-lg bg-green-700 font-bold ">
            <AiOutlineExclamationCircle className="h-6 w-6" />
            <span>All Training</span>
          </NavLink>

          <NavLink to="/admin/exam" className="flex items-center space-x-2 text-gray-300 hover:text-white px-4 py-2 rounded-lg">
            <AiOutlineExclamationCircle className="h-6 w-6" />
            <span>All exam</span>
          </NavLink>
      ...
      <h1 className="text-2xl font-bold mb-6">All training centers</h1>
      {loading ? (
            ...
            <tr>
                  ...
                  <th className="py-2 px-4 text-left">tutorial Name</th>
              ...
              <tbody>
                {training.map((user, index) => (
                  <tr key={user._id} className="border-b">
                    <td className="py-2 px-4">{index + 1}</td>

                    <td className="py-2 px-4">{user.trainingName}</td>
