...
import GroupImage from "../../assets/groupImage[1].png";
  ...
  <Link  to={`/Group/${slug}`}
    ...
    <div className="card bg-white rounded-lg shadow-md overflow-hidden">
      ...
      <div className="card-content p-4">
        ...
        <button className="join-button bg-NavClr px-5 py-1 rounded-md text-white bg-red-600 text-sm transition-all  bg-Blud  duration-300 ease-out hover:scale-105 hover:bg-red-800">
          Join group
        </button>
      </div>
    </div>
    ...
    const fetchGroups = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_VERCEL}/api/groups/getAllGroups`
        );
  ...
  return (
    <div className="w-full px-4">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-3xl text-center font-semibold">Groups you may like</h2>
        <button
