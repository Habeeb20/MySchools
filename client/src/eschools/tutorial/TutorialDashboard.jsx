...
import SlidingText2 from "./SlidingText2";
...
//get my dashboard data
  useEffect(() => {
    const getMyDashboard = async () => {
      ...
      try {
        ...
        const response = await axios.get(`${import.meta.env.VITE_API_TU}/dashboard`, {
          headers: {
  ...
  const SubmitStoreName = async (e) => {
    ...
    try {
      ...
      await axios.post(
        `${import.meta.env.VITE_API_TU}/posttutorialdata`,
        { tutorialName },
        { headers: { Authorization: `Bearer ${token}` } }
      );

      setSuccessMessage("tutorial center name saved successfully!");
  ...
  const getMyStoreData = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(`${import.meta.env.VITE_API_TU}/gettutorialdata`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(
        `${import.meta.env.VITE_API_TU}/${userId}`,
        userProfile,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const sections = {
              ...
              <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 px-4">
                <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
                  <h2 className="text-xl font-semibold text-gray-800 text-center mb-4">
                    Welcome! Please Enter Your Tutorial Name
                  </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex flex-col">
                ...
                <input
                  type="text"
                  name="tutorialName"
                  value={userProfile.tutorialName || ""}
    ...
    <p className="text-lg text-gray-800 mb-2">
      <span className="font-bold">Tutorial Name:</span> {getMyStore.tutorialName}
    </p>
    <p className="text-lg text-gray-800 mb-2">
      <span className="font-bold">Email:</span> {getMyStore.email}
    </p>
    <p className="text-lg text-gray-800 mb-2">
      <span className="font-bold">Price of form:</span> {getMyStore.formPrice}
    </p>
    <p className="text-lg text-gray-800 mb-2">
    ...
    <p className="text-xl text-gray-700">
      ...
      <span className="font-semibold text-black">{getMyStore.exam1}</span>
      <span className="font-semibold text-black">{getMyStore.exam2}</span>
      <span className="font-semibold text-black">{getMyStore.exam3}</span>
      <span className="font-semibold text-black">{getMyStore.exam4}</span>
      <span className="font-semibold text-black">{getMyStore.exam5}</span>
      <span className="font-semibold text-black">{getMyStore.otherclasses}</span>
  ...
  return (
    <div className="flex bg-gray-200 min-h-screen">
      ...
      >
        <h2 className="text-2xl font-bold p-4">Tutorial Dashboard</h2>
        ...
        <div className="relative">
          ...
          <div className="absolute bottom-0 left-6 transform translate-y-1/2 flex items-center space-x-4">
            ...
            <SlidingText2 />
              ...
              <p className="text-black">Tutorial center Name:  {getMyStore.tutorialName}</p>
            </div>
          </div>
        </div>
