...
import SlidingText7 from "./SlidingText7";
const BookshopDashboard = () => {
  ...
  const menuItems = ["Profile", "Feed", "Photos", "Edit_Profile"];
    ...
    const getMyDashboard = async () => {
      ...
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_B}/dashboard`,
          {
            headers: {
  ...
  const SubmitStoreName = async (e) => {
    ...
    try {
      ...
      await axios.post(
        `${import.meta.env.VITE_API_B}/postbookdata`,
        { bookshopName },
  ...
  const getMyStoreData = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(
        `${import.meta.env.VITE_API_B}/getbookdata`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.put(`${import.meta.env.VITE_API_B}/${userId}`, userProfile, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const sections = {
    ...
    Edit_Profile: (
      <div>
        {error && (
          <div className="text-red-500 text-2xl font-semibold mt-5">
            {error}
          </div>
        )}
        {successMessage && (
          <div className="text-green-500 text-2xl font-semibold mt-5">
            {successMessage}
          </div>
        )}
        <div className="flex-col-3 space-x-3">
            ...
            <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 px-4">
              <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
                <h2 className="text-xl font-semibold text-gray-800 text-center mb-4">
                  Welcome! Please Enter Your BookShop Name
                </h2>
                ...
                <form onSubmit={SubmitStoreName} className="space-y-4">
                  <input
                    type="text"
                    value={bookshopName}
                    onChange={(e) => setBookshopName(e.target.value)}
                    required
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="flex flex-col">
                    ...
                    <input
                      type="text"
                      name="bookshopName"
                      value={userProfile.bookshopName}
                  ...
                  <div className="flex flex-col">
                    ...
                    <input
                      type="text"
                      name="BookOrLibrary"
                      value={userProfile.BookOrLibrary}
                      onChange={handleChange}
                      className="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    />
                  </div>
          ...
          <div className="bg-gray-100 rounded-lg shadow-md p-4 mb-6 w-full text-center">
            <p className="text-lg text-gray-800 mb-2">
              <span className="font-bold">Book shop/ Library Name:</span>{" "}
              {getMyStore.bookshopName}
            </p>
            <p className="text-lg text-gray-800 mb-2">
              <span className="font-bold">Bookshop or library:</span>{" "}
              {getMyStore.BookOrLibrary}
            </p>
            <p className="text-lg text-gray-800 mb-2">
              <span className="font-bold">Email:</span> {getMyStore.email || userData.email}
            </p>
            <p className="text-lg text-gray-800 mb-2">
  ...
  return (
    <div className="flex  min-h-screen">
      ...
      >
        <h2 className="text-2xl font-bold p-4">Book shop Dashboard</h2>
      ...
      <div
        className={`flex-1 transition-all ${
          isOpen ? "ml-64 opacity-20" : "ml-0 opacity-none"
        } md:ml-64`}
      >
        {/* Profile Header */}
        <div className="relative">
          ...
          <div
            ...
            style={{
              backgroundImage: `url('https://source.unsplash.com/random/800x300?landscape')`,
            }}
          ></div>
