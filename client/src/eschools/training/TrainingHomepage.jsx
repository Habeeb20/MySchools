...
import CountTraining from './CountTraining'
...
const TrainingHomePage = () => {
    ...
    useEffect(() => {
        const fetchSchools = async () => {
            try {
              const response = await fetch(`${import.meta.env.VITE_API_TR}/getalltraining`);
        ...
        {filteredCards.map((card, index) => (
          ...
          >
            ...
            <div className="p-4">
              ...
              <Link to={`/training/${card.slug}`}>
                <button
                  className="bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-600"
                  onClick={async () => {
                    try {
                      await axios.post(
                        `${import.meta.env.VITE_API_TR}/${card.slug}/click`
                      );
