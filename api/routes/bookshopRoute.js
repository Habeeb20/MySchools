...
import Book from "../models/Eschools/Book/bookshop.schma.js";

const bookshoprouter = express.Router()


bookshoprouter.get("/dashboard", verifyToken, async(req, res) => {
...
bookshoprouter.post("/postbookdata", verifyToken, async (req, res) => {
    try {
        const userId = req.user.id; 
        const { bookshopName } = req.body;
        ...
        // Check if the user has already registered a bookshop
        const existingBookshop = await Book.findOne({ userId });
        if (existingBookshop) {
            console.log("User has already registered a bookshop");
            return res.status(400).json({ message: "User has already registered a bookshop" });
        }

        // Create a new bookshop entry
        const newBookshop = new Book({
            userId,
            bookshopName,
        });
        ...
        // Send response
        res.status(201).json({ message: "Bookshop registered successfully", bookshop: newBookshop });

    } catch (error) {
        console.error("Error registering bookshop:", error);
        res.status(500).json({ message: "Internal Server Error" });
    }
});


bookshoprouter.get("/getbookdata", verifyToken, async(req, res) => {
    try {
       
        console.log("User ID:", req.user?.id);

        const exam = await Book.findOne({ userId: req.user.id });
        if (!exam) {
            console.log("Book shop  not found for user:", req.user.id);
...
bookshoprouter.get("/getallbooks", async(req, res) => {
    try {
        const exam = await Book.find({})
        return res.status(200).json(exam)
    } catch (error) {
        console.error(error);
        return res.status(500).json({ message: "An error occurred" });
...
bookshoprouter.put("/:id", verifyToken, async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        // Find the store
        const store = await Book.findById(id);
        if (!store) {
        ...
        const updatedStore = await Book.findByIdAndUpdate(
            id,
...
//get share
bookshoprouter.get("/:slug/shares", async (req, res) => {
    try {
        const { slug } = req.params;
        const exam = await Book.findOne({ slug }); 
...
bookshoprouter.post("/:slug/shares", async (req, res) => {
    try {
        const { slug } = req.params;
        const store = await Book.findOneAndUpdate(
            { slug }, 
            { $inc: { shares: 1 } },
            { new: true }
        );
...
//increment click
bookshoprouter.post("/:slug/click", async(req, res) => {
    try {
        const {slug} = req.params;
        const store = await Book.findOneAndUpdate(
            {slug},
...
bookshoprouter.get("/get-clicks/:slug", async(req, res) => {
    try {
        const {slug} = req.params;

        const store = await Book.findOne({slug:req.params.slug})
...
bookshoprouter.get("/get-clicks", async (req, res) => {
    try {
      const store = await Book.find({}, "bookshop clicks"); 
  ...
  //get a exam slug
bookshoprouter.get("/abook/:slug", async (req, res) => {
    ...
    try {
      ...
      const store = await Book.findOne(slug);
  
      if (!store) {
        console.log(" not found");
        return res
          .status(404)
          .json({ success: false, message: "bookshop not found" });
      }
...
//post comment
bookshoprouter.post("/:slug/comments", async (req, res) => {
    ...
    try {
        const store = await Book.findOne({ slug: req.params.slug }); // Fixed query
        if (!store) {
...
bookshoprouter.get("/countbookshop", async (req, res) => {
    try {
...
//location count
bookshoprouter.get('/location/counts', async (req, res) => {
    try {
      ...
      for (const state of states) {
        const count = await Book.countDocuments({ state: { $regex: new RegExp(`^${state}$`, 'i') } });
